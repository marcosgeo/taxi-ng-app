...
class TestWebSocket:
    async def test_can_connect_to_server(self, settings):
        ...
        _, access = await create_user('test.user@example.com', 'pAsswOrd')
    ...
    async def test_join_driver_pool(self, settings):
        ...
        message = {
            'type': 'echo.message',
            'data': 'This is a test message on join driver to a pool',
        }
    ...
    async def test_driver_alerted_on_request(self, settings):
        ...
        user, access = await create_user('test.user@example.com', 'pAsswOrd', 'rider')
        communicator = WebsocketCommunicator(
            application=application,
            path=f'/taxi/?token={access}'
        )
        ...
        assert response_data['id'] is not None
        assert response_data['rider']['username'] == user.username
        assert response_data['driver'] is None  # No driver accepted the trip yet
    ...
    async def test_create_trip_group(self, settings):
        ...
        message = {
            'type': 'echo.message',
            'data': 'This a test message for trip group',
        }
        channel_layer = get_channel_layer()
    ...
    async def test_join_trip_group_on_connect(self, settings):
        ...
        message = {
            'type': 'echo.message',
            'data': 'This is a test message for join trip group on connect',
        }
        channel_layer = get_channel_layer()
