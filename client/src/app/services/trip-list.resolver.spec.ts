...
describe('TripListResolver', () => {
  const routerStateSnapshotMock = jasmine.createSpyObj('RouterStateSnapshot', [
    'toString',
  ]);

  it('should resolve a list of trips', () => {
    const tripsMock = [createFakeTrip(), createFakeTrip()];
    ...
    tripListResolver
      .resolve(new ActivatedRouteSnapshot(), routerStateSnapshotMock)
      .subscribe((trips: ReadonlyArray<Trip>) => {
